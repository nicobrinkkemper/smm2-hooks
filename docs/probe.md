...
- `field`: read after the call, relative to the hook's `x0`. Up to 24 per
  ...
  `@@0x21aa468784` is an absolute address, not relocated: it is the object's
  own base, for heap objects that sit at the same address every launch (the
  camera state and its trackers do on this build). Unlike `@addr` (a pointer
